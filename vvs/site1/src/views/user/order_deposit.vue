<template>
    <div class="order-deposit-container">
        ...
        <div class="desposit-fee-style deposit-all clearfix">
            <div class="deposit-fee-tip fl">
                <p class="tip">办理押金即视为消费，不能再申请退款</p>
                <p class='tip new' v-if='newVersion'>押金办理成功后，持二维码可直接到租赁区租赁雪具</p>
            </div>
            <div class="fr">
        ...
        <p class="desposit-fee-style custome-fee"><span class="fee-tip">消费押金</span><input type="number" :value='customeFee' @change='switchCustomeFee($event)' :data-fee='customeFee'></p>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            minFee () {
                return this.orderInfo.minFee ? this.orderInfo.minFee * this.depositCount : 0
            },
            customeFee () {
                return this.userInputFee || this.minFee
            },
            feeAll () {
        ...
        mounted () {
            ...
            if (!cookie.hasCookie('oldVersion')) this.newVersion = true
            this.initData()
        },
        methods: {
            initData () {
                ...
                Api.getOrderDetail({ orderId: this.orderId, verification: true })
                    .then((response) => {
                        ...
                        this.customeFee = this.orderInfo.minFee ? this.orderInfo.minFee : 0
            ...
            switchCustomeFee (e) {
                ...
                if (this.orderInfo.minFee && (fee < this.minFee)) {
                    errorMsg = tips.LOWER_MIN_FEE + `${this.minFee}元`
                }
                if (errorMsg) {
                    Toast({
                        message: errorMsg,
