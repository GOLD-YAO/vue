<template>
    <div class="user-detail-info-container">
        <shop-header :title='infoTitle'>
            <span slot='left' @click='goBack()'><i class="fa fa-angle-left fa-2x"></i></span>
        </shop-header>
        <div class="detail-info-row" v-for='field in info.fields'>
            ...
            <div class="detail-input-container">
                ...
                <select class="detail-input" :data-key='field.name' v-else-if="getType(field.type, 'select')">
                    <option v-for='(value, key) in field.valueArr' :value="key" :selected="value.selected">{{value.title}}</option>
                </select>
            </div>
        </div>
        <div class="more-info-container" v-if='!detail'>
            <span v-if='!info.moreInfoFromThird'>
                <router-link class='more-info-link' :to="{path: '/userinfo', query: {detail: 1}}">个人详细信息</router-link>
            </span>
            <span v-else>
                <a class='more-info-link' :href="info.moreInfoFromThird">个人详细信息</a>
            </span>
        </div>
        <div class="save-userinfo-container">
            <span class="save-userinfo-btn" @click='saveInfo()'>保存</span>
        </div>
        <mt-datetime-picker
...
<script>
    ...
    export default {
        ...
        computed: {
            infoTitle () {
                return this.detail ? '个人详细详细' : '个人基本信息'
            }
        },
        mounted () {
            this.initField()
        ...
        methods: {
            initField () {
                this.detail = this.$route.query.detail ? 1 : 0
                Api.getUserField({ detail: this.detail })
                    .then((response) => {
            ...
            saveInfo () {
                var userInfo = this.serializa()
                var data = {
                    baseInfo: !this.detail,
                    userInfo: userInfo
                }
                console.log(data)
